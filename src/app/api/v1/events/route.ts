...
export const POST = async (req: NextRequest) => {
    try{
        ...
        if(!user.discordId){
            return NextResponse.json({message: "Enter discord ID in your settings."}, { status:403 })
        }
        ...
        if(quota && quota.count >= quotaLimit){
            return NextResponse.json({message: "Monthly quota reached. Switch to the pro plan."}, {status: 429})
        }
        ...
        if(!category){
            return NextResponse.json({message: `You don't have ${category} category` }, {status: 404})
        }
        ...
        return NextResponse.json({message: "Event processed sucessfully", eventId: event.id})
