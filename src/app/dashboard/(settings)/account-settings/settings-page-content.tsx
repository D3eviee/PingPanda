    ...
    return (
        <Card className="max-w-xl w-full space-y-4">
            <div>
                ...
                <Input className="mt-1" value={discordId} onChange={(e)=>{setDiscordId(e.target.value)}} placeholder="Discord ID"/>
            </div>

            <p className="mt-2 text-sm/6 text-gray-600">Don't know how to find your Discord ID? {" "} 
                <Link href="#" className="text-brand-600 hover:text-brand-500">Learn how to obtain it here</Link>.
            </p>

            <div className="pt-4">
                <Button onClick={()=>{mutate(discordId)}} disabled={isPending}>{isPending ? "Saving..." : "Save"}</Button>
            </div>
