...
import { ArrowLeft, ArrowRight, ArrowUpDown, BarChart } from "lucide-react"
...
export const CategoryPageContent = ({hasEvents: initialHasEvents, category}: CategoryPageContentProps) => {
    ...
    const columns: ColumnDef<Event>[] = useMemo(() => [
        {
            ...
            header: "Category",
            cell: () => <span>{category.name || "Uncatorized"}</span>
        },
        {
        ...
        })) : []),
        {
            ...
            cell: ({row}) => (<span className={cn("px-2 py-1 rounded-full text-xs font-semibold", 
    ...
    const NumericFieldSumCards = () => {
        ...
        return Object.entries(numericFieldSums).map(([field, sums])=>{
            const releventSum = activeTab === "today" ? sums.today : activeTab === "week" ? sums.thisWeek : sums.thisMonth

            return (
                <Card key={field}>
                    <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <p className="text-sm/6 font font-medium">{field.charAt(0).toUpperCase() + field.slice(1)}</p>
                        <BarChart className="size-4 text-muted-foreground"/>
                    </div>

                    <div>
                        <p className="text-2xl font-bold">{"$" +  releventSum.toFixed(2)|| 0}</p>
                        <p className="text-2xs/5 text-muted-foreground">{activeTab === "today" ? "today" : activeTab === "week" ? "this week" : "this month"}</p>
                    </div>
                </Card>
            )
        })
       ...
       <div className="space-y-6">
            ...
            <TabsContent value={activeTab}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
                    <Card className="border-2 border-brand-700">
                        <div className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <p className="text-sm/6 font font-medium">Total events</p>
                            <BarChart className="size-4 text-muted-foreground"/>
                        </div>

                        <div>
                            <p className="text-2xl font-bold">{data?.eventsCount || 0}</p>
                            <p className="text-2xs/5 text-muted-foreground"> Events{" "}{activeTab === "today" ? "today" : activeTab === "week" ? "this week" : "this month"}</p>
                        </div>
                    </Card>
        ...
        <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
                <div className="w-full flex felx-col gap-4">
