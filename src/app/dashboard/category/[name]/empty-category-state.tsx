...
export const EmptyCategoryState = ({categoryName}: {categoryName: string}) => {
    ...
    const codeSnippet =  `await fetch('https://localhost:3000/api', {
  method: 'POST',
  headers: {
  ...
  body: JSON.stringify({
    ...
    return(
        <Card contentClassName="max-w-2xl w-full flex flex-col items-center p-6" className="flex-1 flex items-center justify-center">
            <h2 className="text-xl/8 font-medium text-center tracking-tight text-gray-950">Create your first {categoryName} event</h2>
            <p className="text-sm/6 text-gray-600 mb-8 max-w-md text-center text-pretty">Get started by sending reqest to our tracking API: </p>
