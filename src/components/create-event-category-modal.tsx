...
export const CreateEventCategoryModal = ({children, containerClassName}: CreateEventCategoryModal) =>{
    ...
    const { register, handleSubmit, setValue, watch, formState: {errors}} = useForm<EventCategoryForm>({
    ...
    return (
        ...
        <Modal showModal={isOpen} setShowModal={setIsOpen} className="max-w-xl p-8">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div>
                    <h2 className="text-large/7 font-medium tracking-tight text-gray-950">New Event Category</h2>
                    <p className="text-sm/6 text-gray-600">Create a new category to orginize your events.</p>
                </div>
                        ...
                        <div className="flex flex-wrap gap-3 mt-2">
                            {COLOR_OPTIONS.map((colorOption) => (
                                <button 
                                    key={colorOption} 
                                    type="button" 
                                    className={cn(`bg-[${colorOption}]`, 
                                        "size-10 rounded-full ring-2 ring-offset-2 transition-all", 
                                        color === colorOption ? "ring-brand-700 scale-110" : "ring-transparent hover:scale-105") }
                        ...
                        <div className="flex flex-wrap gap-3 mt-2">
                            {EMOJI_OPTIONS.map(({emoji, label}) => (
                                <button 
                                    key={emoji} 
                                    type="button" 
                                    className={cn("size-10 flex items-center justify-center text-xl rounded-md transition-all", 
                ...
                <div className="flex justify-end space-x-3 pt-4 bordet-t">
                    <Button type="button" variant="outline" onClick={()=>{setIsOpen(false)}}>Cancel</Button>
                    <Button type="submit" variant="outline"  disabled={isPending} onClick={()=>{setIsOpen(false)}}>{isPending ? "Creating" : "Create category"}</Button>
                </div>
            </form>
        </Modal>
        </>
